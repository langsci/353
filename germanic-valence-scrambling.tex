...
Subsection~\ref{sec-cp-tp-vp}, Subsection~\ref{sec-down-to-arth-syntax} is optional and it is
...
\label{sec-down-to-arth-syntax}

Chapter~\ref{chap-psg} relied on constituent tests that are standardly assumed in the syntactic
...
the verb. In fact, the reason for these features to be assumed in grammars is that they play a role
...
correspond to parts of speech and morpho-syntactic features like gender, number, and case. The \xbar
grammar introduced in Chapter~\ref{chap-psg} follows this tradition, which goes back to
\citet{Bloomfield26a}. 
...
module. Figure~\ref{Abb-T-Modell} shows the so-called T~model.
\begin{figure}
\centering
\begin{forest}
...
syntax they would be verb phrases (VPs) or sentences (S). Figure~\ref{Abbildung-Remnant-Movement-Satzstruktur} shows an analysis of a German clause in this
tradition. The syntax tree contains a wild mix of categories including a Topic Phrase (TopP),
...
The model of syntax argued for in this book is very simple: categories stand for linguistic objects
with certain properties that belong to the same distribution class. For example an NP with masculine
gender in the third person singular (\ref{ex-np-tisch}) or an \nbar with masculine gender in the third person
singular as in (\mex{1}).
\eal
\ex 
\gll Vase\\
     vase\\
...
\gll alte Vase aus China\\
     old  vase from China\\
\zl
Since the phrases/words in (\mex{-1}) and (\mex{0}) share the respective properties they can be
exchanged. These categories can be selected by other heads. As was described in this chapter, a verb
may select an NP in the nominative. This is not possible in theories with ``creative'' categories. I
will explain this with respect to \citegen{Cinque99a-u} adverb analysis but there are analyses that
make the problem even more obvious. The next subsection will deal with an analysis of coordination
that assumes that the conjunction is a head projecting a Conjunction Phrase (ConjP).
...
do not exist in
the language under consideration \citep[\page 55]{CR2010a}. This of course requires a rather strong conception of Universal
...
like case, gender and even nationality \citep[\page 96, 99, 100]{Cinque94a-u} should enter the human genome\footnote{
  See and \citet{Bishop2002a} and \citet*{EBJKSPP96a} about genetics and linguistic information. See
  also \citet*{HCF2002a}.
...
approach does not work without further add ons, as the following example by \citet{Haider2022a}\addpages shows:
\ea
...
attached to VPs whose head has different verb forms. These verb forms are selected by the governing
...
empty adverbial heads. The actual adverbs are analyzed as specifiers of these heads.
\begin{figure}
\oneline{%
\begin{forest}
sm edges
                      ...
                      [F-AdvP
                        ...
                        [F-Adv$'$
                          ...
                          [{VP[\textit{pas}]}
                            ...
                            [NP$_i$ [\trace]]]]]]]]]]]]]]]]
\end{forest}}
\caption{VP cascade including empty functional head hosting adverbial phrases in specifier positions
...
developing a theory of extended projections suggested something along these lines. One could have a
...
\textsc{major} and \textsc{minor} head features \citep[Section~9.3.1]{Netter94}.}  \citet[\page 122]{Grimshaw2000a} also states that symmetric coordination could be handled this
way. She states that in coordinations functional and lexical information has to be identified. But
...
\z
Figure~\ref{fig-a-squirrel-DP} shows the analysis of the phrase \emph{the president} in the DP
  ...
  [D$'$
     ...
     [NP
       [N$'$
         [N [president]]]]]] 
\end{forest}
\caption{\emph{a squirrel} in the DP analysis}\label{fig-a-squirrel-DP}
\end{figure}
Now, one could project information about the lexical category as well. This would make information
about verbs and nouns available at the top node of the projection. Let us assume the feature \textsc{f} for
functional categories and the feature \textsc{l} for lexical categories. 
...
correct part of speech and the correct verb form by looking at the \textsc{l} value. But this does
not extend to the coordination problem. If the conjunction is treated as a functional head, the
...
Note also that verb phrases with different adverbials may be coordinated:
\ea
...
here -- is semantic or information structural information within syntactic categories.
...
Now, returning to the \citeauthor{CR2010a} style analyses. \citet{Ernst92a} examined an analysis of
...
solve these problems by using constraints that can search in trees for information and find out that
the NegP \emph{not always} contains an adverb, that the NegP \emph{not have heard the news} contains
a verb in the base form and that \emph{not heard the news} contains a verb in the participle perfect
form, but any such approach would be more complicated than the approach taken here and all other
non-Minimalist theories (for explicit discussion see \citealt{Sag2007a}) and also many Minimalist theories (\eg \citealt[\page
  223]{Abraham2005a}). The approach that was discussed in this chapter just compares a requirement
  of a head with the properties of the complement or specifier daughter. Or the other conjunct in
  the case of examples like (\ref{ex-coordination-NegP}). In comparison, \citegen{Laenzlinger2004a}
  approach has to find the verb in \emph{nicht oft gut gespielt hat} `not often well played has'
  somewhere deeply embedded in a cascade of NegP, AspP, MannP, AuxP, Aux+. Note also that it is
...
are requiring NPs or VPs.\footnote{
  ...
  below in Section~\ref{sec-major-minor-category}. They will not work either. Furthermore, the coordination of two singular NPs is a plural NP not a singular NP as would
  be predicted by \citeauthor{Johannessen96a-u}'s account. See \citet{Borsley2005a} for
  details. Approaches assuming two category features will be discussed in Subsection~\ref{sec-funct-proj-and-adverbs}
}
\eal
...
\zl
Note also that the example in (\mex{0}b) shows that the assumption that a negated VP is a VP rather
than a NegP provides for a simple analysis of coordination: it is just VPs that are coordinated and
hence (\mex{0}b) is an instance of symmetric coordination. In a NegP approach, more would have to be
said about matching categories in the coordination.
...
classes. Coordination is one of these tests and if we find to conjoinable constituents this
indicates that they should be assigned to the same category.
\citet[\page 106]{Cinque99a-u} suggests the following hierarchy of functional heads.
\ea
...
Consider the sentence in (\mex{1}):
\ea
...
are combined with (at least) 27 empty heads and are (at least) 27fold ambiguous. When two such VPs are
...
that do not add semantics, spurious ambiguities result in coordinations.

\subsubsection{Reaching Cinque's goals}
...
pointed out to me a long time ago: they are entertaining some kind of Construction Grammar
...
generation'': rather than moving constituents they are licensed in the places they are visible.
...
is Minimal Recursion Semantics. All semantic contribution are contained in lists of elementary
predications. Such lists can be augmented by a pointer pointing to the elementary predication that
was added by the last adverbial. Flickinger uses such a pointer. It is called \textsc{altkey}. Since
a modifier selects the VP it modifies, it can also access the value of \textsc{altkey} and hence a
