...
within embedded clauses. The ratio is 10 subjects compared to 15 non-subjects. So, the question is: What does
...
is basic would not be appropriate. Rather Adv V S O should be regarded as a basic pattern if we
assume this little text as our empirical basis. Of course, assuming the two texts as the basis of
...
subjects in the \vf in 50.3\,\% and 52.1\,\% of the cases, respectively. 
...
order. Therefore syntacticians of various different frameworks (see \citealp{MuellerGT-Eng4} for
...
infront of V in the V2 languages has nothing to do with the SVO/SOV dichotomy and basically disturbs
the picture and makes the counting-approach pursued in the WALS (see Section~\ref{sec-intro-svo}) non-applicable. 
...
added at the end of the clause. In the corresponding Danish example from \citet[\page 146]{Oersnes2009b}, it is exactly the other way
...
\gll  at   Conny ikke [\sub{VP} læser bogen]\\
      that Conny not      {}        reads          book.\textsc{def}\\\jambox{(Danish)}
...
\gll  Conny læser ikke bogen.\\
       Conny reads   not  book.\textsc{def}\\\jambox{(Danish)}
...
\gll Liest Conny das Buch?\\
     reads Conny the book\\ \jambox{(German)}
\glt `Does Conny read the book?'
\z
...
\gll Har$_i$ Conny [ \_$_i$ læst bogen]?\\
     has Conny {} {} read book.\textsc{def}\\\jambox{(Danish)}
\glt `Has Conny read the book?'
\ex
\gll Hat$_i$ Conny das Buch [gelesen \_$_i$]?\\
     has Conny the book \spacebr{}read\\ \jambox{(German)}
\glt `Has Conny read the book?'
\zl
...
This section deals with verb first sentences in Subection~\ref{sec-verb-first} and then covers verb
second sentences in Subsection~\ref{sec-verb-second}.
...
interested reader may consult the discussion section~\ref{sec-discussion-scope}.
