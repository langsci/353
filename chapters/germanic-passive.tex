...
This chapter deals with the passive\is{passive|(}. The passive is usually analyzed as the suppression of the
...
determine whether \emph{hennar} `her' and \emph{Þeim} `them' are subjects or not. The reason is that
these elements are fronted and both subjects and objects can be fronted. However, \citet*{ZMT85a} argued that
...
\footnotetext{    
\url{http://www.photovoltaikforum.com/sds-allgemein-ueber-solar-log-f38/solarlog-1000-mit-wifi-anschliesen-t96371.html}, 2014-01-10.
}
...
\gll Avisen        læstes           hver   dag.\\
     newspaper.\textsc{def} read  every day\\
...
\gll Avisen skal læses hver dag.\\
     newspaper.\textsc{def} must read.\textsc{inf}.\textsc{pass} every day\\\danish
...
principle assumed here differs from the one suggested by \citeauthor*{YMJ87} in not assigning case to elements that are raised
...
\zl
Those analyses that do not distinguish between structural and lexical case would have to assume that the case of the objects
...
\ex[*]{
\gll das geschlafene Kinf\\
     the slept child\\\hfill(unergative)
}
\zl
...
\itdopt{Add explanation that all inflected forms are derived by lexical rules}
...
\begin{tabular}[t]{@{}l@{ }ll@{ }l@{}}
  &                              & \textsc{arg-st}\\[2mm]
a.&getanzt     (danced, unerg):  & \eliste\\[2mm]
b.&gelesen     (read, trans):    & \sliste{ NP[\type{str}] }\\[2mm]
c.&geschenkt   (given, ditrans): & \sliste{ NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&geholfen    (helped, unerg):  & \sliste{ NP[\type{ldat}] }\\
\end{tabular}
%}
\z
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & \textsc{arg-st}\\[2mm]
a.&danse   (dance, unerg):    & \sliste{ NP[\type{str}]$_i$ }\\[2mm]
b.&læse      (read, trans):   & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$ }\\[2mm]
c.&give      (give, ditrans): & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[2mm]
d.&hjælpe    (help, trans):   & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$ }\\
\end{tabular}
%}
\z
...
(\ref{ex-fordi-manden-giver-barnet-bolden}) -- repeated as (\mex{1}a) for convenience --, the two sentences in (\mex{1}b,c) would be admitted: 
\eal
\ex[]{
\gll fordi manden giver barnet bolden\\ 
     because man.\textsc{def} gives child.\textsc{def} ball.\textsc{def}\\\danish
...
\zl
(\mex{0}b) is ungrammatical with \emph{barnet} `child' as the recipient of the giving. Similarly, the
...
a.&danset/-s   (dance, unerg):     & \eliste\\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.&læst/-s      (read, trans):   &  \sliste{ NP[\type{str}]$_j$ } \\[2mm]
c.&givet/-s      (give, ditrans): & \sliste{ NP[\type{str}]$_j$, NP[\type{str}]$_k$ } \\[2mm]
  &                         & \sliste{ NP[\type{str}]$_k$, NP[\type{str}]$_j$ } \\[2mm]
d.&hjulpet/-s    (help, trans):   & \sliste{ NP[\type{str}]$_j$ }                    \\
\end{tabular}
%}
\z
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & \textsc{arg-st}\\[2mm]
a.&dansa   (dance, unerg):     & \sliste{ NP[\type{str}] }\\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.& lesa      (read, trans):   & \sliste{ NP[\type{str}], NP[\type{str}] }\\[2mm]
c.&gefa       (give, ditrans): & \sliste{ NP[\type{str}], NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&hjálpa     (help, trans):   & \sliste{ NP[\type{str}], NP[\type{ldat}] }\\
\end{tabular}
%}
\z
...
%a.&ankomme (unacc):       & \eliste                         & \sliste{ NP[\type{str}]}\\[2mm]
a.& dansað    (danced, unerg):     & \eliste                        & \eliste & \eliste \\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.& lesið      (read, trans):   &  \sliste{ NP[\type{str}]$_j$ }         & \sliste{ NP[\type{str}]$_j$ } & \eliste\\[2mm]
c.& gefið      (given, ditrans): & \sliste{ NP[\type{ldat}]$_j$, NP[\type{str}]$_k$ } & \sliste{ NP[\type{ldat}]$_j$ } & \sliste{ NP[\type{str}]$_k$ }\\[2mm]
  &                      & \sliste{ NP[\type{str}]$_k$, NP[\type{ldat}]$_j$ } & \sliste{ NP[\type{str}]$_k$ } & \sliste{ NP[\type{ldat}]$_j$ }\\[2mm]
d.& hjálpað    (helped, trans):   & \sliste{ NP[\type{ldat}]$_j$ }                  & \sliste{ NP[\type{ldat}]$_j$ } & \eliste\\
\end{tabular}%
}
\z
...
then be mapped to the \sprl \citep{MuellerDanishImpersonalPassive}. (\mex{1}) shows the mapping to
\spr and \comps of the version of \emph{danset} `danced' that underwent this lexical rule:
\ea
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{~~~~~}l@{}}
  &                        & \textsc{arg-st}                     & \spr   & \comps\\[2mm]
...
\begin{tabular}[t]{@{}l@{ }l@{~~}l@{ }l@{~~}l@{}}
  &                              & \textsc{arg-st} & \spr & \comps\\[2mm]
a.&getanzt     (danced, unerg):  & \eliste        & \eliste & \eliste\\[2mm]
b.&gelesen     (read, trans):    & \sliste{ NP[\type{str}] } & \eliste & \sliste{ NP[\type{str}] }\\[2mm]
c.&geschenkt   (given, ditrans): & \sliste{ NP[\type{ldat}], NP[\type{str}] } & \eliste & \sliste{ NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&geholfen    (helped, unerg):  & \sliste{ NP[\type{ldat}] } & \eliste & \sliste{ NP[\type{ldat}] }\\
\end{tabular}}
\z
...
reader is referred to \citew{Mueller96a}, \citew[Section~2.2.2]{Mueller2002b}, or \citew[Section~15.2]{MuellerLehrbuch4}.
...
passive in GB is different though.  \textcites[155--157]{Grewendorf88a}[\page 1311]{Grewendorf93} and
...
fulfilling the need of a subject. This insertion is done via a lexical rule, which also applies for
expletive insertion in intransitive constructions.

The SVO languages seem to require different items for the perfect/passive participles, but \citegen{Haider86}
passive analysis for \ili{German} using just one participle form for both perfect and passive can be
maintained for German and also be extended to other SOV languages.
