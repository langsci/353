...
Claiming that SVO is a basic order of German on the basis of pure counting is somehow strange given the
...
order.\footnote{
The clauses in (i) are in SOV order. (i.d) is non-finite and does not have a subject.
\eal
\ex 
...
During the discussion of scope facts, I already hinted at an analysis of German clauses in which a \isi{trace} marks the
position of the verb in final position and the verb in initial position is coindexed with this
trace. Although the SVO languages differ from SOV languages like German, a similar analysis has been suggested for languages
...
in the framework of \isi{Categorial Grammar} for \ili{English}. \citet{Borsley89} adapted this analysis for HPSG
...
that correspond to those of non-fronted elements in the initial \mf position. That is, (\mex{1}a)
corresponds to (\mex{1b}) rather than (\mex{1}c):
\eal
\ex
\gll Das Buch liest Conny.\\
...
\glt `Does Conny read the book.'
\zl
This means that (\mex{0}a) patterns with (\mex{-1}a) rather than with (\mex{-1}b).
...
combined with the verb, if they appear to the right of the verb, that is, in positions in which
complements usually are located.
\begin{figure}
\begin{forest}
sm edges
[S
