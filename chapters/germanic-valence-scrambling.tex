...
\eal
\ex 
\gll Ich unterstütze  \emph{ihn}.\\
     I.\NOM{} support him.\ACC\\\german
\ex 
\gll Ich helfe \emph{ihm}.\\
     I.\NOM{} help him.\DAT\\
\ex 
...
element in the valence list first and then with all the other elements \citep[347--348]{Borsley89b}. However, nothing has been
...
three-element list with \type{nom}, \type{dat} and \type{acc} elements, this would be the case in (\mex{0}c) and \ibox{2} would be NP[\type{acc}] and
...
\end{forest}}
\caption{\label{fig-nobody-gave-the-child-a-book}Analysis of a sentence with a ditransitive verb}
\end{figure}
...
specifying linguistic constraints \parencites[Section~3]{AB2021a}[Section~2]{Richter2021a}. Values
of features like part of speech are types and types can have
...
\noindent
For \ili{German} adverbials, the \modv restricts the part of speech of the head to be verb (or rather
verbal since, as (\mex{1}b) shows, adjectival participles can be modified as
well) and the value of \textsc{initial} to be $-$. 
\eal
\ex
\gll dass das Kind oft lacht\\
     that the child often laughs\\\german
\glt `that the child laughs often'
\ex 
...
modified node is a VP, the value of \textsc{mod} is not given in detail in the figures below, but is just shared with the
...
\z
\cont abbreviates \textsc{content} in (\mex{0}). Its value represents the semantic contribution of a
...
combined with \parencites[296--297]{Jacobs94a}[393]{Jacobs2003a-u}. So, for example, the verb
\emph{lesen} `to read' can be used with an object or without it.
\eal
\ex 
...
\end{tabular}
\z
The respective analyses are shown in Figure~\ref{fig-optional-argument}.
\begin{figure}
\hfill
\scalebox{.8}{%
\begin{forest}
...
\end{forest}}
\hfill\mbox{}
\caption{\label{fig-optional-argument}Analysis of sentences with bi-valent and mono-valent \emph{liest} `reads'}
\end{figure}
...
Both options are suggested in the literature: \citet[52]{Michaelis2012a} and \citet[23]{Flickinger2000a} suggested a construction for
...
\ili{German} developed in the theory of Government \& Binding (GB; \citealt{Chomsky81a,Chomsky86b}). Subsection~\ref{sec-down-to-earth-syntax} contains a comparison to
...
Minimalism (so-called Mainstream Generative Grammar, MGG\indexmgg) but also in frameworks like LFG (\citealp[Section~6.2]{Bresnan2001a}; \citealp[Section~3.2.1]{Dalrymple2001a-u}). However,
...
Figure~\ref{fig-cp-tp-vp-scrambling}.\footnote{%
  ...
  \begin{NoHyper}\parencites{Larson88a}[\page 331]{Adger2003a}\end{NoHyper} are also forced to assume movement to account for
...
p.~\pageref{bsp-grammatik-psg} -- repeated here as (\mex{1}) for convenience:\todostefan{Das ist eine andere Grammatik.}
\ea
...
approaches.\todostefan{fix this} But Cartographic approaches face further problems, some of which will be
discussed here.
...
\gll Laut dem Bürgermeister wird man wahrscheinlich in Zukunft nur am SAMSTAG dieses Gebäude besichtigen können.\\ 
     according the mayor will one probably in future only on Saturday this building visit can\\
...
that topics can stay in their normal positions despite being topics, as \citet[148]{Fanselow2006a}
pointed out:
\largerpage[2]
\ea
...
\z
\emph{Eine Maklerin} and \emph{die Prinzessin} are both feminine NPs and both could be nominative or
accusative. If the NP \emph{die Prinzessin} is placed to the left of \emph{eine Maklerin}, the reading changes so that the princess cheated the
      ...
      [Mood\sub{evaluative}\rlap{$'$}
        ...
        [Mood\sub{evidential}P
          ...
          [Mood\sub{evidential}\rlap{$'$}
            ...
            [\ldots]]]]]]]
\end{forest}
\caption{Topmost part of \citegen[106]{Cinque99a-u} functional hierarchy of adverbial projections}\label{fig-cinque-top}
\end{figure}
