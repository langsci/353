...
\itdopt{AZ: put in footnote?}A note of caution is necessary here: especially the following three subsections are potentially
...
is not always identical to the language-particular definitions. 
\itdopt{AZ: i think you say the opposite of what you mean: according to the semantic definition there is no subject in that sentence} For instance, if one follows the
...
finiteness/non-finiteness distinction into account cannot tease these properties apart \citep[Section~3]{Hoehle83a}. In what follows we will have a look at main and embedded clauses with
...
\itdopt{AZ: this leaves the Dutch situation out of the picture, where the object is like German before the verbs the the different verbs come in the Danish order or in the following order dat zij hem moet gezien hebben}
...
evidence thathas been used in the literature to argue for the OV status of languages like German and Dutch.
...
\parencites[\page 113]{dBMvW86a-u}[\page 388]{Diesing97a}{Sadock98a-u} or an OV
language \parencites{Hall79a-u}[\iaddpages]{Vikner2001a-u}. Some researchers argued that it is
neither but a third type of language, one with mixed VO/OV status \parencites{Santorini93a-u}[\page 12]{Schallert2007a}[\page 161]{Haider2010a}{Haider2020a}.
...
\z
\itdopt{AZ: why is there a ? with this sentence. It is not a direct question.}
...
%\zl
\itdopt{AZ: a bit cryptic: which variation} So the variation is \emph{w}-phrase + SOV, \emph{w}-phrase + SVO, and \emph{w}-phrase + V2.
...
Danish uses the expletive to make it clear that an extraction of a constituent took place \itdopt{AZ: this assumes a lot of analysis that has not been provided yet. Maybe rephrase: Danish uses an expletive in sentences such as ... In Chapter ?? we will show that the language here avoids string vacuous movement.}
...
\zl
\itdopt{AZ: this comes out of the left field. Maybe say: arguments that in German IHM is not a subject will be reviewed in ...} According to the tests that were used for languages like Icelandic (for instance the possibility to
omit a subject in infinitival constructions), the dative object \emph{ihm} `him' in (\mex{0}a) is
not a subject. In (\mex{0}b), there is no nominal argument at all. \itdopt{AZ: i would postpone this discussion: first do German and Danish and then Icelandic} As we will discuss in
