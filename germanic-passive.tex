...
\gll Hvenær hafði Sigga        hjálpað Haraldi?\\
     when   has   Sigga.\NOM{} helped Harold.\DAT{}\\\icelandic
}
\ex[*]{
...
\gll Mich     dünkt  der Mann langweilig.\\
     I.\ACC{} thinks the.\NOM{} man boring\\\german
...
\subsubsubsection{Nominatives and accusative objects}

Until now the case that an argument gets assigned by its head was represented in the valence list of
...
genitive. So, nominative, genitive, and accusative are structural cases in German. (The question
...
\subsubsubsection{Genitive objects}
...
\subsubsubsection{Dative objects}
...
We now saw how the participle forms of the languages under considerations look like and how they are
...
Passive auxiliary for Germanic languages (\emph{be}, \emph{werden}, \emph{blieve}, etc.):
\avm{
...
Auxiliaries in the SVO langauges embed a VP. This means that the \compsv of the embedded verb has to
...
\stem{geb} and licenses the particple form \emph{gegeben}. \emph{gegeben} \iboxb{3} is combined with
...
Unfortunately \citeauthor{Haider86}'s approach does not work for SVO langauges. If we wanted to use
the argument blocking/deblocking approach, we had to assume the structures in (\mex{1}a--b):
\eal
...
\ea
\gll weil    der        Wagen oft   [[zu reparieren versucht] wurde].\\
     because the.\NOM{} car   often \hspaceThis{[[}to repair   tried     was\\
...
object. The dativ object is coindexed with the subject of the embedded verb, that is, the two NPs
...
Englisch, German, and Icelandic. The lexical rule accounts both for the morphological and the
analytical passives. The first element on the \argstl is suppressed and a relational constraint
\emph{promote} promotes any NP with structural case. The languages differ in what cases they use and
