...
This chapter deals with the passive.\itdopt{
This chapter is a rough draft. Most of the references to other literature are still missing \ldots
} The passive is usually analyzed as the suppression of the
...
Genitives and datives as in (\mex{1}) are not counted among the subjects in German.
\eal
\ex 
...
The German equivalent would be the sentence in (\mex{1}):
\ea[??]{
...
verb in the question in (\mex{1}) and hence in subject position.
...
\gll Ich hoffe hier geholfen zu bekommen.\footnotemark\\
     I   hope  here helped   to get\\\german
\footnotetext{    
...
\z
 ...
 \subsection{Comparison German, Danish, English, Icelandic}
...
% ZMT85a:443 Icelandic also has some morphologically'middle' forms in the suffix -st, someof which
...
Danish has a morphological passive. It is formed by appending the suffix \suffix{s} to the verb, and there
...
\noindent
As the following examples show, the subject can be an S or a VP:\todostefan{show that these are really
...
impersonal passive.
} Since German does not require a subject, impersonal passives like (\mex{1}) are expected:
...
The following two examples from Icelandic show that Icelandic also has impersonal constructions \citep[\page 264]{Thrainsson2007a-u}:
\eal
\ex 
...
(\mex{1}) shows.
\ea[*]{
fordi manden giver bolden barnet
}
\z
...
\emph{ambáttin} `the female slave' is the subject in (\mex{1}b).
\eal
\ex
...
in (\mex{1}a) and the nominative in (\mex{1}b) is realized immediately following the finite verb.
\eal
...
Let us now turn to the dative. If we look at examples like (\mex{1}), we see that the dative does not
...
impersonal passives and nominalizations.\footnote{\label{fn-structural-dative}
  ...
  \citep[\page 96]{Gunkel2003b} or additional features singling out accusative objects
...
\gll "`Klärle"' hätte es wirklich mehr als verdient, auch mal zu einem "`unrunden"' Geburtstag gratuliert zu bekommen.\footnotemark\\
     \hspaceThis{"`}Klärle had it really more than deserved also once to a \hspaceThis{"`}insignificant birthday congratulated to get\\
\footnotetext{
...
%der dann noch viel später bekannte: "Ich hab' immer noch Gänsehaut, das war der schönste Teil meines Geburtstages." Doch auch die anderen Abteilungen des Bürgervereins können auf ein erfolgreiches Jahr 1998 zurückblicken.
     with the old song   by  Elvis Presley \hspaceThis{"`}I can't help falling in love got cashier Markus Reiß to.the birthday congratulated\\
\footnotetext{
...
%% \subsubsubsection{Non-cannonical accusative objects}
...
\oneline{%
\begin{tabular}[t]{@{}l@{~}l@{~}l@{}}
a. & \emph{lässt} with \emph{schlafen}:      & \argst \sliste{ NP[\type{str}]$_l$, NP[\type{str}]$_i$, V}\\[2mm]
b. & \emph{lässt} with \emph{unterstützen} : & \argst \sliste{ NP[\type{str}]$_l$, NP[\type{str}]$_i$, NP[\type{str}]$_j$, V }\\[2mm]
c. & \emph{lässt} with \emph{helfen}:        & \argst \sliste{ NP[\type{str}]$_l$, NP[\type{str}]$_i$, NP[\type{ldat}]$_j$, V }\\[2mm]
d. & \emph{lässt} with \emph{schenken}:      & \argst \sliste{ NP[\type{str}]$_l$, NP[\type{str}]$_i$, NP[\type{ldat}]$_j$, NP[\type{str}]$_k$, V }\\
\end{tabular}
}
\z
...
\zl
(\mex{0}a) is ungrammatical with \emph{barnet} `child' the recipient of the giving. Similarly the
...
passive sentences. Let's start with an English example. The analysis of (\mex{1}) is shown in
...
The analysis of the parallel German sentence in (\mex{1}) is shown in Figure~\vref{fig-dem-Kind-ein-Roman-gegeben-wurde}.
\ea
...
\citegen{Haider86} analysis of the passive is brilliant since it is sufficient to have one lexical
...
The so-called \emph{remote passive}\is{passive!remote} is a highlight of German syntax since several
phenomena interact in a non-trivial way. It was first discussed by \citet[\page
175--176]{Hoehle78a}. Höhle observed that objects of German infinitives with \emph{zu} appear in the
...
English \citep[\page 124]{Chomsky81a} to German (see also \citealt[\page 180]{Lohnstein2014a} for a
...
scrambling in this system). The discussion of this analysis of the passive is based on \citew[Section~3.4]{MuellerGT-Eng4}.
...
\eal
\ex[]{
The dolfin gave [the child] [a ball].
}
\ex[]{
...
be the normal order for passives and (\mex{0}b) is derived from this by movement of \emph{dem
  Kind} `the child', (\mex{0}b) should be more marked than (\mex{0}c), contrary to the facts. To
...
this kind of empty element, see \citew[Section~13.1.3 and Chapter~19]{MuellerGT-Eng4} and \citew[Section~7]{MuellerHeadless}.

\citet[\page 12]{Koster86a} has pointed out that the passive in English\il{English} cannot be derived by Case
Theory, that is, lack of case and movement to the specifier position of TP due to the Case Filter, since if one allowed empty expletive subjects for English as well as German and Dutch\il{Dutch}, then it would be possible
...
\itdopt{Add GB and Gunkel and Ryu}
